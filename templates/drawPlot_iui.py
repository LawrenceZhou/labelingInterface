...
for c in conditions:
    ...
    for i in range(10):
        ...
        """
        if True:
            with open('experiment_result/' + c + '_variation_sdtw_eucli_trial_' + str(i) + '.csv', 'rU') as data:
                reader = csv.reader(data)
                j = 1
                file = open('experiment_result/' + c + '_variation_sdtw_eucli_trial_' + str(i) + '.csv')
                numline = len(file.readlines())
                if numline != 49:
                    continue
                for row in reader:
...
for c in conditions:
  for j in range(1, 50):
    ...
    for i in range(len(cluster_sdtw[c][str(j)])):
      if j != 1:
        cluster_eucl[c][str(j)][i]= (cluster_eucl[c][str(j)][i] + 0.5 * cluster_eucl[c][str(j - 1)][i]) / 1.5
      if j != 1:
        cluster_sdtw[c][str(j)][i]= (cluster_sdtw[c][str(j)][i] + 0.5 * cluster_sdtw[c][str(j - 1)][i]) / 1.5
